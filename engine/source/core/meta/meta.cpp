module crumb.core.meta;

import std;

// ============================================================================
// 反射运行时实现 — 全局注册表 + 访问器 + 序列化器基础类型特化
// 移植自 Piccolo reflection.cpp / serializer.cpp
// ============================================================================
...
int TypeMeta::getFieldsList(FieldAccessor*& out_list) {
    ...
    for (int i = 0; i < count; ++i) { out_list[i] = m_fields[i]; }
    return count;
}
...
int TypeMeta::getMethodsList(MethodAccessor*& out_list) {
    ...
    for (int i = 0; i < count; ++i) { out_list[i] = m_methods[i]; }
    return count;
}
